...
def get_modules():
    """Scan docs/modules for module directories and extract titles."""
    modules_dir = Path("docs/modules")
    modules = []
    
    if not modules_dir.exists():
        print(f"Warning: {modules_dir} does not exist")
        return modules
    
    # Find all mod-* directories
    for mod_dir in sorted(modules_dir.glob("mod-*")):
        ...
        # Make title lowercase for nav display
        title = title.lower()
        
        modules.append({
            'title': title,
            'path': f"modules/{mod_dir.name}/README.md"
        })
